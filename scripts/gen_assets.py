...
# ── Logo ──────────────────────────────────────────────────────────────────
# book/assets/logo.png is now a real designed asset (committed directly,
# not generated by this script) -- gen_logo() below is kept only as the
# placeholder that shipped before the real logo existed. It is NOT called
# from __main__ anymore, specifically so a routine `python
# scripts/gen_assets.py` regeneration run can never silently clobber the
# real logo with this placeholder again.

def _placeholder_gen_logo_do_not_call_from_main():
...
if __name__ == "__main__":
    # logo.png intentionally NOT regenerated here -- see the note above
    # _placeholder_gen_logo_do_not_call_from_main(). It's a real designed
    # asset now, not a build output.
    gen_core_loop()
    gen_meta_stack()
    gen_three_strikes()
